...
class AddStriker extends Component
{
    ...
    $finishing, $dribbling, $header, $attitude, $photo, $i = 0, $x = 0, $validateTeam, $upload_path = "", $candidateId, $editedTeam, $edit_mode_index = null;
    public $file;
    public $filepath = "";
    public $success = 0;
    ...
    {   
        if ($this->searchTeam !== null) {
            $this->teamsArray = Team::where('team_name','like', '%' . $this->searchTeam . '%')->where('user_id', Auth::user()->id)->get()->toArray();
        }else{
            $this->teamsArray = Team::where('user_id', Auth::user()->id)->get()->toArray();
        }

        if ($this->searchCandidate !== null) {
            $alternatif =  Alternatif::where('name','like', '%' . $this->searchCandidate . '%')->where('team_id', $this->team_id)->paginate(5);
        }else{
